...
package pl.wcislo.sbql4j.tools.javac.processing;

import static sbql4jx.tools.StandardLocation.ANNOTATION_PROCESSOR_PATH;
...
import pl.wcislo.sbql4j.source.util.TaskEvent;
import pl.wcislo.sbql4j.source.util.TaskListener;
...
import pl.wcislo.sbql4j.tools.javac.model.JavacTypes;
import pl.wcislo.sbql4j.tools.javac.parser.Keywords;
...
import pl.wcislo.sbql4j.tools.javac.util.Paths;
import sbql4jx.annotation.processing.Filer;
import sbql4jx.annotation.processing.Messager;
import sbql4jx.annotation.processing.ProcessingEnvironment;
import sbql4jx.annotation.processing.Processor;
import sbql4jx.annotation.processing.RoundEnvironment;
import sbql4jx.lang.model.SourceVersion;
import sbql4jx.lang.model.element.AnnotationMirror;
...
import sbql4jx.lang.model.util.ElementScanner6;
import sbql4jx.lang.model.util.Elements;
import sbql4jx.tools.DiagnosticListener;
...
public class JavacProcessingEnvironment implements ProcessingEnvironment,
		...
		public Processor next() {
			try {
				...
				if ("ServiceConfigurationError".equals(t.getClass()
					...
					log.error("proc.processor.constructor.error", t
							.getLocalizedMessage());
				}
				throw new Abort(t);
			}
				...
				if (!names.hasNext())
					...
					try {
						try {
							...
							log
									.error("proc.processor.not.found",
											processorName);
							return false;
		...
		runAround: while (true) {
			...
			if (messager.errorRaised()) {
				...
				if (moreToDo()) {
					...
					for (JavaFileObject jfo : filer
							.getGeneratedSourceFileObjects()) {
						fileObjects = fileObjects.prepend(jfo);
					}

					compiler = JavaCompiler.instance(currentContext);
					List<JCCompilationUnit> parsedFiles = compiler
							.parseFiles(fileObjects);
	...
	private void runLastRound(PrintWriter xout, int roundNumber,
			boolean errorStatus, TaskListener taskListener) throws IOException {
		roundNumber++;
	...
	/**
	 * For internal use by Sun Microsystems only. This method will be removed
	 * without warning.
	 */
