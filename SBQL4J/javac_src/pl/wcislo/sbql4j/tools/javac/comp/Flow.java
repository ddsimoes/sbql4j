...
public class Flow extends TreeScanner {
    ...
    //ew
    public boolean isInit(DiagnosticPosition pos, VarSymbol sym) {
        if ((sym.adr >= firstadr || sym.owner.kind != TYP) &&
                trackable(sym) &&
                !inits.isMember(sym.adr)) {
        	return false;
 ...
 *************************************************************************/
    ...
    public void visitSbqlExpression(JCSbqlExpression that) {
    	for(Iterator<Symbol.VarSymbol> it = that.localVarContext.iterator(); it.hasNext(); ) {
    		Symbol.VarSymbol varS = it.next();
    		boolean isFlowOk = isInit(that, varS);
    		if(!isFlowOk) {
    			it.remove();
    		}
    	}
