...
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ABSTRACT;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ANNOTATION;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.BLOCK;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.COMPOUND;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.DEPRECATED;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ENUM;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.FINAL;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.GENERATEDCONSTR;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.HASINIT;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.INTERFACE;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.NATIVE;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.NOOUTERTHIS;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.PROPRIETARY;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.PUBLIC;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.STATIC;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.UNATTRIBUTED;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.VARARGS;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.AMBIGUOUS;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.ERR;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.ERRONEOUS;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.MTH;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.NIL;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.PCK;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.TYP;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.VAL;
import static pl.wcislo.sbql4j.tools.javac.code.Kinds.VAR;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.ARRAY;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.BYTE;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.CLASS;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.ERROR;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.FORALL;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.INT;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.METHOD;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.PACKAGE;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.TYPEVAR;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.VOID;
import static pl.wcislo.sbql4j.tools.javac.code.TypeTags.WILDCARD;
...
import pl.wcislo.sbql4j.lang.views.info.JCViewInfoCollector;
import pl.wcislo.sbql4j.lang.views.info.ViewAnalysisHelper;
import pl.wcislo.sbql4j.lang.views.info.ViewInfo;
import pl.wcislo.sbql4j.source.tree.IdentifierTree;
import pl.wcislo.sbql4j.source.tree.MemberSelectTree;
import pl.wcislo.sbql4j.source.tree.TreeVisitor;
import pl.wcislo.sbql4j.source.util.SimpleTreeVisitor;
import pl.wcislo.sbql4j.tools.javac.code.BoundKind;
import pl.wcislo.sbql4j.tools.javac.code.Flags;
import pl.wcislo.sbql4j.tools.javac.code.Kinds;
import pl.wcislo.sbql4j.tools.javac.code.Lint;
import pl.wcislo.sbql4j.tools.javac.code.Scope;
import pl.wcislo.sbql4j.tools.javac.code.Source;
import pl.wcislo.sbql4j.tools.javac.code.Symbol;
import pl.wcislo.sbql4j.tools.javac.code.Symbol.ClassSymbol;
import pl.wcislo.sbql4j.tools.javac.code.Symbol.CompletionFailure;
import pl.wcislo.sbql4j.tools.javac.code.Symbol.MethodSymbol;
import pl.wcislo.sbql4j.tools.javac.code.Symbol.OperatorSymbol;
...
import pl.wcislo.sbql4j.tools.javac.code.Type.ErrorType;
import pl.wcislo.sbql4j.tools.javac.code.Type.ForAll;
import pl.wcislo.sbql4j.tools.javac.code.Type.MethodType;
...
import pl.wcislo.sbql4j.tools.javac.jvm.ByteCodes;
import pl.wcislo.sbql4j.tools.javac.jvm.Target;
...
import pl.wcislo.sbql4j.tools.javac.tree.JCTree.JCCompilationUnit;
...
import pl.wcislo.sbql4j.tools.javac.tree.JCTree.JCExpression;
...
import pl.wcislo.sbql4j.tools.javac.tree.JCTree.JCStatement;
...
import pl.wcislo.sbql4j.tools.javac.tree.TreeInfo;
import pl.wcislo.sbql4j.tools.javac.tree.TreeMaker;
import pl.wcislo.sbql4j.tools.javac.util.Context;
import pl.wcislo.sbql4j.tools.javac.util.JCDiagnostic;
import pl.wcislo.sbql4j.tools.javac.util.JCDiagnostic.DiagnosticPosition;
...
import pl.wcislo.sbql4j.tools.javac.util.Options;
import pl.wcislo.sbql4j.tools.javac.util.Position;
import pl.wcislo.sbql4j.tools.javac.util.Warner;
 ...
 *  
 *  @author Emil Wcislo (SBQL query tree)
 */
 ...
 *************************************************************************/
     ...
     */
    public Type attribTree(JCTree tree, Env<AttrContext> env, int pkind, Type pt) {
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        ...
        try {
            ...
            if (tree.body == null) {
                ...
                // in a retrofit mSignature class.
    ...
    public void visitVarDef(JCVariableDecl tree) {
        if(tree.init instanceof JCSbqlExpression) {
        	JCSbqlExpression sbqlExpr = (JCSbqlExpression) tree.init;
        	sbqlExpr.resultIsAssigned = true;
        }
...
//          System.out.println(clazz + " generated.");//DEBUG
    ...
    public void visitAssign(JCAssign tree) {
        ...
        if(tree.rhs instanceof JCSbqlExpression) {
        	JCSbqlExpression sbqlExpr = (JCSbqlExpression) tree.rhs;
        	sbqlExpr.resultIsAssigned = true;
        }
    }
...
//                             Resolve.isStatic(env));//DEBUG
    ...
    @Override
    public void visitSbqlExpression(JCSbqlExpression that) {
    	if(that.attrVisited) {
    		return;
    	}

    	JavaNameResolver jnr = new JavaNameResolver(rs, env, that.pos, names);
    	ClassTypes.initInstance(syms, names, jnr, types);
//        JavaNameResolver jnr = new JavaNameResolver(rs, env, 0, names);
        
    	
//    	Signature s0 = jns.findIdent("employees");
//    	Signature s1 = jns.findIdent("emp1");
//    	Signature s2 = jns.findIdent("pl");
    	try {
    		ViewInfo vInfo;
    		Type classType = env.enclClass.type;
    		if(classType != null) {
    			vInfo = ((ClassType)classType).viewInfo;
    		} else {
    			vInfo = that.enclosingClass.viewInfo;
    		}
        	
    		boolean enclosingClassIsViewDef = vInfo != null;
    		that.viewQuery = enclosingClassIsViewDef;
    		JCMethodDecl mth = env.enclMethod;
//    		boolean isMthOnRead = enclosingClassIsViewDef && mth.getName().toString().equals(JCViewInfoCollector.MTH_NAME_ON_READ);
//    		boolean isMthGetSeed = enclosingClassIsViewDef && mth.getName().toString().equals(JCViewInfoCollector.MTH_NAME_VIRTUAL_OBJECTS);
    		boolean isMthOnRead = enclosingClassIsViewDef
    			&& vInfo.getOnReadInfo() != null 
    			&& mth == vInfo.getOnReadInfo().getOnReadMethodDecl();
    		boolean isMthGetSeed = enclosingClassIsViewDef 
    			&& vInfo.getSeedInfo() != null 
    			&& mth == vInfo.getOnReadInfo().getOnReadMethodDecl();
    		java.util.List<BinderSignature> viewContext = new ArrayList<BinderSignature>();
    		if(isMthOnRead) {
    			//need to inject result of getSeed method to context of the query
    			if(vInfo.getSeedInfo().getSeedSignature() == null) {
    				visitSbqlExpression(vInfo.getSeedInfo().getSeedJCExpression());
    			}
    			viewContext = vInfo.getVirtualObjectsSignaturesWithParentViews();
    		} else if(isMthGetSeed && vInfo.getParentViewInfo() != null) {
    			//need to inject result of getSeed method 
    			//of parent views to context of the query
    			ViewInfo pViewInfo = vInfo.getParentViewInfo();
    			if(pViewInfo.getSeedInfo().getSeedSignature() == null) {
    				visitSbqlExpression(pViewInfo.getSeedInfo().getSeedJCExpression());
    			}
    			viewContext = pViewInfo.getVirtualObjectsSignaturesWithParentViews();
    		}
    		//check if enclosing class is view definition, in case of non-standalone view
    		//typecheck is ommited
    		boolean checkTypes = true;
//    		if(enclosingClassIsViewDef && !vInfo.isStandalone()) {
//    			checkTypes = false;
//    		}
    		SBQL4JStatement checkStatement = new SBQL4JStatement(that.expr, jnr, that.pos, checkTypes, viewContext);
	    	Signature resultType = checkStatement.staticCheck();
	    	if(isMthGetSeed || isMthOnRead) {
	    		if(resultType instanceof BinderSignature) {
	    			BinderSignature bSig = (BinderSignature) resultType;
	    			if(isMthGetSeed) {
	    				vInfo.getSeedInfo().setSeedSignature(bSig);
	    			} else if(isMthOnRead) {
	    				vInfo.getOnReadInfo().setOnReadSignature(bSig);
	    			}
	    		} else {
	    			//exception - result should be binder signature
	    		}
	    	}
	    	ClassTypes.initInstance(syms, names, jnr, types);
	    	if( !checkStatement.isViewUsed() ) {
		    	checkStatement.optimise();
		    	checkStatement.decorateNames();
	    	}
	    	if(resultType == null) {
//	    		log.error(key, args)
	    	}
	    	that.resultType = resultType;
	    	that.localClassContext = jnr.getResolvedJavaTypes();
	    	that.localVarContext = jnr.getResolvedJavaVars();
	    	that.localPackageContext = jnr.getResolvedJavaPackages();
	    	that.exprTree = checkStatement.getExpressionTree();
	    	that.usedJNR = jnr;
	    	that.attrVisited = true;
    	} catch(ParserException e) {
    		log.error(that.pos + e.getPos() + 1, "sbql.syntax", e.getToken()); //
    	}

    
    	/**
    	 * poniewaz uzywamy staycznej analizy, niepotrzebne jest 'wrzucanie' wszystkich zmiennych i klas
    	 */
    	/*
		Env<AttrContext> env1 = env;
    	AttrContext info = env.info;
    	java.util.List<Symbol> nieWiemCoZTymZrobic = new ArrayList<Symbol>();
    	java.util.List<Symbol.VarSymbol> localContext = new ArrayList<Symbol.VarSymbol>();
    	
    	
    	java.util.List<Symbol.ClassSymbol> localClassContext = new ArrayList<Symbol.ClassSymbol>();
//    	rs.printscopes(env);
    	while(env1 != null) {
    		Iterable<Symbol> it = env1.info.getLocalElements();
	    	for(Symbol s : it) {
	    		if(s instanceof Symbol.VarSymbol) {
	    			Symbol.VarSymbol vs = (Symbol.VarSymbol)s;
	    			//tutaj musimy uwzglednic zmienne uzyte w statycznym otoczeniu
	    			boolean stat = s.isStatic();
	    			boolean local = s.isLocal();
	    			int envStaticLev = env.info.staticLevel;
	    			long flags = s.flags();
	    			String flagsStr = Flags.toString(flags);
	    			int kind = s.kind;
	//    			boolean rsAccesible = rs.isAccessible(s, that, env.enclMethod.type, s.name, true, null, null);
//	    			Symbol rsSymbol = rs.findIdent(env, s.name, s.kind);
	//    			boolean flowAccesible = flow.isInit(that, vs);
	//    			if(stat || local || (!stat && envStaticLev == 0)) { 
	    			JCIdent ident = new JCIdent(vs.name, vs);
	    			ident.pos = that.pos;
	    			noreport = true;
	    			rs.noreport = true;
	    			visitIdent(ident);
	    			noreport = false;
	    			rs.noreport = false;
	    			boolean error = this.error || rs.foundError;
	    			this.error = false;
	    			rs.foundError = false;
	    			if(!error) {
	    				 if (env.enclClass.sym.owner.kind != PCK && vs.name == names._super) { // we are in an inner class
	    					 //odrzucamy super w klasie wew
	    				 } else {
	    					 //odrzucamy duplikaty
		    				 boolean foundDuplicate = false;
		    				 for(VarSymbol v : localContext) {
		    					 if(v.name == vs.name) {
		    						 foundDuplicate = true;
		    						 break;
		    					 }
		    				 ...
		    				 if(!foundDuplicate) {
		    					 localContext.add((Symbol.VarSymbol)s);
		    				 }
	    				 }
	    				
	    				 
	    			
//	    			if(!(rsSymbol instanceof ResolveError) && !localContext.contains(s)) {
	    				
//	    			}
	    			}
	    		} else {
	    			nieWiemCoZTymZrobic.add(s);
	    		}
	    	}
	    	List<JCTree> classMembers = env.enclClass.getMembers();
	    	for(JCTree cm : classMembers) {
	    		if(cm instanceof JCVariableDecl) {
	    			JCVariableDecl var = (JCVariableDecl) cm;
	    			Symbol.VarSymbol s = var.sym;
	    			Symbol rsSymbol = rs.findIdent(env, s.name, s.kind);
	    			if(!(rsSymbol instanceof ResolveError)) {
	    				localContext.add(var.sym);
	    			}
	    		}
	    	}
	...
	//    	for(Symbol s : envClass.info.getLocalElements()) {
	//    		if(s instanceof Symbol.ClassSymbol) {
	//    			localClassContext.add((Symbol.ClassSymbol)s);
	//    		} else {
	//    			nieWiemCoZTymZrobic.add(s);
	//    		}
	//    	}
    	...
    	that.localVarContext = localContext;
//    	java.util.List<JCImport> imports = envClass.toplevel.getImports();
//    	java.util.List<Symbol> namedImport = new ArrayList<Symbol>();
    	Env<AttrContext> envClass = env;
    	// wczytanie klas zaimportowanych explicite (np. import java.util.ArrayList)
    	for(Symbol s : envClass.toplevel.namedImportScope.getElements()) {
    		if(s instanceof Symbol.ClassSymbol && !localClassContext.contains(s)) {
    			localClassContext.add((Symbol.ClassSymbol)s);
    		} else {
    			nieWiemCoZTymZrobic.add(s);
    		}
...
//    	java.util.List<Symbol> starImport = new ArrayList<Symbol>();
    	
    	//wczytanie klas zaimportowanych implicite (np. import javax.crypto.*, domyslny import java.lang.*)
    	for(Symbol s : envClass.toplevel.starImportScope.getElements()) {
    		if(s instanceof Symbol.ClassSymbol && !localClassContext.contains(s)) {
    			localClassContext.add((Symbol.ClassSymbol)s);
    		} else {
    			nieWiemCoZTymZrobic.add(s);
...
//    		starImport.add(s);
    	...
    	//wczytanie klas zaimportowanych implicite z tego samego pakietu
    	Scope pckScope = env.toplevel.packge.members_field;
    	for(Symbol s : pckScope.getElements()) {
    		if(s instanceof Symbol.ClassSymbol && !localClassContext.contains(s)) {
    			localClassContext.add((Symbol.ClassSymbol)s);
    		} else {
    			nieWiemCoZTymZrobic.add(s);
    	...
    	for(Symbol.ClassSymbol s : localClassContext) {
    		if(s.name.toString().contains("JGrammarTest1")) {
    			System.out.println(s);
    		}
    	}
    	//sprawdzamy dostepnosc klas
    	java.util.List<Symbol.ClassSymbol> toDel = new ArrayList<Symbol.ClassSymbol>();
    	for(Symbol.ClassSymbol s : localClassContext) {

    		if(s.owner instanceof Symbol.ClassSymbol || s.type.tsym.owner instanceof Symbol.ClassSymbol) {
    			boolean isOwnerClass = s.owner instanceof Symbol.ClassSymbol;
    			if(isOwnerClass) {
    				int x = 6;
    			}
//    			Symbol sym = rs.findIdentInType(env, s.owner.type, s.name, 2);
//    			if(sym instanceof ResolveError) {
    				toDel.add(s);
//    			}	
    		} else {
    			
    			Symbol sym = rs.findIdent(env, s.name, 2);
    			boolean isOwnerClass = s.owner instanceof Symbol.ClassSymbol;
    			if(sym instanceof ResolveError) {
    				toDel.add(s);
    			} else if(isOwnerClass) {
    				toDel.add(s);
    			}
    		}
			
    	}
    	localClassContext.removeAll(toDel);
    	that.localClassContext = localClassContext;
...
//        Env<AttrContext> env = enter.typeEnvs.get(c);
//        JCClassDecl tree = (JCClassDecl) env.tree;
        if(isClassViewDeclaration(tree)) {
        	JCViewInfoCollector viewInfoCollect = new JCViewInfoCollector(tree, (ClassType) tree.type, this);
        	ViewInfo vInfo = viewInfoCollect.analyseViewDeclaration();
        	((ClassType)tree.type).viewInfo = vInfo;
        }
        //-- EW
    ...
    private boolean isClassViewDeclaration(JCClassDecl decl) {
    	if(ClassTypes.getInstance() == null) {
	    	JavaNameResolver jnr = new JavaNameResolver(rs, env, decl.pos, names);
	    	ClassTypes.initInstance(syms, names, jnr, types);
    	}
    	return ViewAnalysisHelper.isClassViewDeclaration(decl);
    }

    /**
     * @author Emil
     */
    private boolean isCurrentClassNotStandaloneView(Env<AttrContext> currentEnv) {
    	JCClassDecl curClass =  currentEnv.enclClass;
    	for(JCAnnotation ann : curClass.mods.annotations) {
    		if(ann.annotationType.type.toString().equals("pl.wcislo.sbql4j.lang.views.View")) {
