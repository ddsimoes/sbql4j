...
public class Pretty extends JCTree.Visitor {
    ...
    public Pretty(Writer out, boolean sourceOutput, CodeGenerator sbqlCodeGen) {
        this.out = out;
        this.sourceOutput = sourceOutput;
        this.sbqlCodeGen = sbqlCodeGen;
    }

    public final CodeGenerator sbqlCodeGen;
    private JCCompilationUnit topLevelTree;
...
//where
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        try {
            ...
            if (tree.name == tree.name.table.init &&
                    (enclClassName == null || enclClassName.len == 0) &&
                    sourceOutput) return;
            println(); align();
            printDocComment(tree);
            printExpr(tree.mods);
    ...
    @Override
    public void visitSbqlExpression(JCSbqlExpression tree) {
        try {
          	String invocationCode = sbqlCodeGen.generateQueryInvocation(tree);
        	print(invocationCode);
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    }
