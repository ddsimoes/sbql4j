...
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ACC_BRIDGE;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ACC_SUPER;
import static pl.wcislo.sbql4j.tools.javac.code.Flags.ACC_VARARGS;
...
import static pl.wcislo.sbql4j.tools.javac.code.Flags.CLASS_SEEN;
...
import static pl.wcislo.sbql4j.tools.javac.code.Flags.EXISTS;
...
import static pl.wcislo.sbql4j.tools.javac.code.Flags.SOURCE_SEEN;
...
import static sbql4jx.tools.StandardLocation.CLASS_PATH;
import static sbql4jx.tools.StandardLocation.PLATFORM_CLASS_PATH;
import static sbql4jx.tools.StandardLocation.SOURCE_PATH;
...
import pl.wcislo.sbql4j.tools.javac.comp.Annotate;
import pl.wcislo.sbql4j.tools.javac.util.BaseFileObject;
...
import sbql4jx.tools.JavaFileManager.Location;
...
public class ClassReader extends ClassFile implements Completer {
    ...
    /** Switch: read GJ mSignature information.
     */
 ...
 ***********************************************************************/
    ...
    /** Read mSignature and convert to type.
     */
    Type readType(int i) {
        int index = poolIdx[i];
    ...
    /** If name is an array type or class mSignature, return the
    ...
    /** Read mSignature and convert to type parameters.
     */
 ...
 ***********************************************************************/
    ...
    /** Convert mSignature to type, where mSignature is a name.
     */
    ...
    /** Convert mSignature to type, where mSignature is a byte array segment.
     */
    ...
    /** Convert mSignature to type, where mSignature is implicit.
     */
    Type sigToType() {
        ...
        case 'L':
            {
                ...
                if (sigp < siglimit && signature[sigp] == '.')
                    throw badClassFile("deprecated inner class mSignature syntax " +
                                       "(please recompile from source)");
                /*
                System.err.println(" decoded " +
                                   new String(mSignature, oldsigp, sigp-oldsigp) +
                                   " => " + t + " outer " + t.outer());
                */
                return t;
            }
    ...
    int sbp = 0;
    /** Convert class mSignature to type, where mSignature is implicit.
     */
    Type classSigToType() {
        if (signature[sigp] != 'L')
    ...
    /** Convert (implicit) mSignature to list of types
    ...
    /** Convert mSignature to type parameters, where mSignature is a name.
     */
    List<Type> sigToTypeParams(Name name) {
        return sigToTypeParams(name.table.names, name.index, name.len);
    }

    /** Convert mSignature to type parameters, where mSignature is a byte
     *  array segment.
     */
    ...
    /** Convert mSignature to type parameters, where mSignature is implicit.
     */
    ...
    /** Convert (implicit) mSignature to type parameter.
     */
    Type sigToTypeParam() {
        int start = sigp;
        while (signature[sigp] != ':') sigp++;
 ...
 ***********************************************************************/
    ...
    protected JavaFileObject preferredFileObject(JavaFileObject a,
        ...
        if (preferSource)
            return (a.getKind() == JavaFileObject.Kind.SOURCE) || (a.getKind() == JavaFileObject.Kind.SOURCE_SBQL4J) ? a : b;
        else {
    ...
    protected EnumSet<JavaFileObject.Kind> getPackageFileKinds() {
        return EnumSet.of(JavaFileObject.Kind.CLASS, JavaFileObject.Kind.SOURCE, JavaFileObject.Kind.SOURCE_SBQL4J);
    }
...
//        	JavacFileManager jcFileManager = (JavacFileManager) fileManager;
//        	jcFileManager.
//        }
        ...
        classKinds.remove(JavaFileObject.Kind.SOURCE_SBQL4J);
