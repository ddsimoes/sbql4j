...
package pl.wcislo.sbql4j.tools.javac.parser;

import static pl.wcislo.sbql4j.tools.javac.parser.Token.AMP;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.ASSERT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.CASE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.CATCH;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.CLASS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.COLON;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.COMMA;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.DEFAULT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.DOT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.ELLIPSIS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.ELSE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.ENUM;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.EOF;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.EQ;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.ERROR;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.EXTENDS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.FINAL;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.FINALLY;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.GT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.GTEQ;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.GTGT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.GTGTEQ;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.GTGTGTEQ;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.IDENTIFIER;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.IMPLEMENTS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.IMPORT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.INSTANCEOF;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.INTERFACE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.INTLITERAL;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.LBRACE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.LBRACKET;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.LONGLITERAL;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.LPAREN;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.LT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.MONKEYS_AT;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.NEW;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.PACKAGE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.PLUSEQ;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.PLUSPLUS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.QUES;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.RBRACE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.RBRACKET;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.RPAREN;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.SEMI;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.STAR;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.STATIC;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.SUB;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.SUBSUB;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.SUPER;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.THROWS;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.TRUE;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.VOID;
import static pl.wcislo.sbql4j.tools.javac.parser.Token.WHILE;
import static pl.wcislo.sbql4j.tools.javac.util.ListBuffer.lb;
...
import pl.wcislo.sbql4j.tools.javac.tree.JCTree.JCModifiers;
...
import pl.wcislo.sbql4j.tools.javac.tree.JCTree.TypeBoundKind;
import pl.wcislo.sbql4j.tools.javac.util.Context;
import pl.wcislo.sbql4j.tools.javac.util.Convert;
 ...
 */
public class Parser {
	private static final org.apache.commons.logging.Log logger = LogFactory.getLog(Parser.class);
...
//where
    ...
    public JCExpression expression() {
    	if(S.token() == Token.HASH) {
//    		logger.debug("Parser.expression() "+Token.HASH);
//    		return sbqlExpression();
    	}
    	
        return term(EXPR);
    }
    
    public JCExpression sbqlExpression() {
    	int startPos = S.pos()+2;
    	S.nextToken();
...
//    	while(S.token() != Token.HASH) {
//    		S.nextToken();
//    	}
    	int endPos = S.pos()-1;
//    	S.nextToken();
//    	accept(SEMI);
    	String expr = new String(S.getRawCharacters(startPos, endPos));
//    	expr = expr.trim();
//    	logger.debug("Parser.sbqlExpression() expr='"+expr+"'");
    	JCSbqlExpression sbqlExpr = F.SbqlExpression(null, expr.toString());
    	sbqlExpr.pos = startPos - 2;
    	return sbqlExpr;
    }
...
//where
    ...
    protected JCExpression term3() {
        ...
        switch (S.token()) {
        case HASH:
//        	logger.debug("Parser.term3() HASH");
        	return sbqlExpression();
        case QUES:
...
//todo: skip to anchor on error(?)
    ...
    public JCStatement statement() {
        ...
        switch (S.token()) {
//        case HASH:
//        	System.out.println("Parser.statement() HASH");
        case LBRACE:
            return block();
        case IF: {
            S.nextToken();
        ...
        default:
            Name name = S.name();
//            if(S.token() == Token.HASH) {
//            	logger.debug("Parser.statement() HASH");
//            }
            JCExpression expr = expression();
