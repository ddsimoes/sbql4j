...
import static pl.wcislo.sbql4j.mirror.apt.Filer.Location.CLASS_TREE;
import static pl.wcislo.sbql4j.mirror.apt.Filer.Location.SOURCE_TREE;
...
public class FilerImpl implements Filer {
    ...
    private enum FileKind {
        ...
        CLASS  {
            void register(File file, String name, FilerImpl that) throws IOException {
                if (that.filesCreated.contains(new File(that.locations.get(SOURCE_TREE), that.nameToPath(name, ".java")))
                ||	that.filesCreated.contains(new File(that.locations.get(SOURCE_TREE), that.nameToPath(name, ".s4j")))	) {
    ...
    private final pl.wcislo.sbql4j.tools.apt.main.JavaCompiler comp;
    ...
    private FilerImpl(Context context) {
        ...
        comp = pl.wcislo.sbql4j.tools.apt.main.JavaCompiler.instance(context);
        roundOver = false;
