 ...
 *  @author Neal Gafter
 */
public class JavadocTool extends pl.wcislo.sbql4j.tools.javac.main.JavaCompiler {
    DocEnv docenv;
    ...
    docenv.quiet = quiet;
        ...
        try {
            for (List<String> it = javaNames; it.nonEmpty(); it = it.tail) {
                String name = it.head;
                if (!docClasses && (name.endsWith(".java") || name.endsWith(".s4j")) && new File(name).exists()) {
                    docenv.notice("main.Loading_source_file", name);
                        JCCompilationUnit tree = parse(name);
                        classTrees.append(tree);
                } else if (isValidPackageName(name)) {
                    names = names.append(name);
                } else if (name.endsWith(".java") || name.endsWith(".s4j")) {
    ...
    private static boolean isValidJavaSourceFile(String file) {
    	boolean javaS = file.endsWith(".java");
    	boolean s4jS = file.endsWith(".s4j");
        if (!(javaS || s4jS)) return false;
        
        String postfix = javaS ? ".java" : ".s4j";
        String clazzName = file.substring(0, file.length() - postfix.length());
        return isValidClassName(clazzName);
    }
