...
public class KORQueries_SbqlQuery8 {
    private com.db4o.ObjectContainer dbConn;

    public KORQueries_SbqlQuery8(final com.db4o.ObjectContainer dbConn) {
        this.dbConn = dbConn;
    }

    /**
     * original query='dbConn.((max(Pilkarz.pensja) as pensja
                            join (max(Pilkarz.premia) as premia)).(pensja+premia))'
     *
     * query after optimization='dbConn.( max(Pilkarz.getPremia()) as premia group as _aux0).( max(Pilkarz.getPensja()) as pensja join _aux0).(pensja + premia)'
    */
    public java.lang.Integer executeQuery() {
        com.db4o.ObjectContainer _ident_dbConn = dbConn;
        java.lang.Integer _queryResult = _ident_dbConn.query(new KORQueries_SbqlQuery8Db4o0());
