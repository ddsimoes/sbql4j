...
public class KORQueries_SbqlQuery22 {
    private com.db4o.ObjectContainer dbConn;

    public KORQueries_SbqlQuery22(final com.db4o.ObjectContainer dbConn) {
        this.dbConn = dbConn;
    }
     ...
     *
     * query after optimization='dbConn.((0 as i close by (i + 1 where i <=  max(PilkarzMecz.getLiczba_czerwonych_kartek())) as i join  count((PilkarzMecz where getLiczba_czerwonych_kartek() >= i and getLiczba_czerwonych_kartek() < i + 1)) as c).(c + " Pilkarzy zdobylo czerwonych kartek  pomiedzy  " + i + " and " + i + 1) as message)'
    */
    public java.util.List<java.lang.String> executeQuery() {
        com.db4o.ObjectContainer _ident_dbConn = dbConn;
        java.util.List<java.lang.String> _queryResult = _ident_dbConn.query(new KORQueries_SbqlQuery22Db4o0());
