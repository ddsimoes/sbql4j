...
public class  Db4oSBQLInterpretedQuery<R> implements Db4oSbqlQuery<R> {
	...
	public Db4oSBQLInterpretedQuery(Expression queryRootNode,
	...
	public R executeQuery(ObjectContainerBase ocb, Transaction t) {
    	...
    	for(String cn : topLevelClassNames) {
    		ClassMetadata cm = clazzRep.getClassMetadata(cn);
    		long[] ids = cm.getIDs(transLocal);
			for(long id : ids) {
				...
				for(pl.wcislo.sbql4j.model.QueryResult q : resC) {
					if(q instanceof JavaComplexObjectReflect) {
						...
						resVar.add(new Binder(cm.getName(), db4oObj));
					} else {
