...
public class ComplexDb4oSbql4j extends Db4oSbql4jDriver implements Complex {
...
//			ObjectSet<ComplexHolder2> result = query.execute();
			Collection<ComplexHolder2> result = Sbql4jQueries.complexQuery(db(), currentInt);
			if(result.size() != 1) {
				throw new IllegalStateException("" + result.size());
			}
			
			ComplexHolder2 holder = (ComplexHolder2) result.iterator().next();
